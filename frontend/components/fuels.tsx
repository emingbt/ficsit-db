          ...
          <div>
            <div className="w-full h-16 sm:h-24 flex flex-row items-center justify-center bg-main-bg p-1 sm:p-2">
              {fuel.items.map((item, j) => (
                <>
                  {item.type === 'PRODUCT_OF' && <div className='w-0.5 h-full sm:h-full bg-main-orange mx-1 sm:mx-2' />}
                  <p className='text-xs sm:text-base'>{parseFloat(item.rate.toFixed(4))}x</p>
                  <div className={`h-8 sm:h-14 aspect-square p-0.5 sm:p-1 m-1 sm:m-2 bg-light-bg
                  ${item.isFluid ? 'rounded-full' : 'rounded-sm sm:rounded-md'}`} key={j}>

...
export function getFilteredItemsByName(name) {
  ...
  const gameData = JSON.parse(jsonData)
  const allItems = gameData.items
  const allBuilables = gameData.buildables

  const inputSlug = name.split(' ').join('-').toLowerCase()

  const items = Object.keys(gameData.items)
    .filter(key => key.includes(inputSlug))
    .reduce((cur, key) => { return Object.assign(cur, { [key]: { slug: allItems[key].slug, name: allItems[key].name } }) }, {})

  const buildables = Object.keys(gameData.buildables)
    .filter(key => key.includes(inputSlug))
    .reduce((cur, key) => { return Object.assign(cur, { [key]:{ ...allBuilables[key], category: allBuilables[key].categories[0] }}) }, {})
